...
const traverse = (nodes: any[], astsInsideWpBlock: any[]): any[] => {
  return nodes.reduce((prev: any, node: any) => {
    ...
    if (node.type === "comment") {
      ...
      if (!astsInsideWpBlock) return [...prev];
      customNode.value = trimmedValue;
      customNode.type = "wpblock";
      customNode.name = "wpblock";
      customNode.root = astsInsideWpBlock[multilineWpBlockCount];
...
export const traverseAstIncludedWpBlock = traverse;
